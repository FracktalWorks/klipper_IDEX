...
# Modified 2025 by Hamil Sunny <hamil@fracktal.in>
#
...
# The hybrid-corexy kinematic is also known as Markforged kinematics. Modified to have same kinematis as coreXY with extra two Y axis steppers for high accels in Y.
class FracktalHybridCoreXYKinematics:
    def __init__(self, toolhead, config):
        self.printer = config.get_printer()
        self.inverted = False
        if config.has_section('fracktal_hybrid_corexy'):
            hcxy_config = config.getsection('fracktal_hybrid_corexy')
...
def load_kinematics(toolhead, config):
    return FracktalHybridCoreXYKinematics(toolhead, config)
